...
const { importUsersFromXlsx } = require('../utils/importUserFromXlsx');
router.post('/import-xlsx', async function (req, res, next) {
  try {
    // Hardcode file path for demo, or get from req.body.filePath
    const filePath = path.join(__dirname, '../uploads/user.xlsx');
    const count = await importUsersFromXlsx(filePath);
    res.send({ message: `Imported ${count} users from xlsx.` });
  } catch (err) {
    res.status(400).send({ message: err.message });
  }
});
